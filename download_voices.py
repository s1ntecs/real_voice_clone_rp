            ...
            for chunk in response.iter_content(chunk_size=CHUNK_SIZE):
                if chunk:
                    ...
                    if total_size > 0:
                        ...
                        print(f"\r  ⏳ Прогресс: {progress:.1f}% ({downloaded}/{total_size} bytes)", end='')  # noqa
...
def process_voice_model(voice_name, url):
    ...
    try:
        ...
        if not model_files_dir:
            print("  ❌ Не найдены файлы модели (.pth) в архиве")
            return False

        print(f"  📁 Найдены файлы модели в: {model_files_dir.relative_to(temp_model_dir)}")  # noqa

        # Перемещаем файлы модели в целевую директорию
        print("  📂 Перемещение в: {target_dir}")
